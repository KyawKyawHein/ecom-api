                ...
                <tr class="border border-slate-400">
                    ...
                    <td class="border p-2 flex items-center gap-3 justify-center">
                        ...
                        <form action="{{ route('orders.accept',$order->id) }}"  method="post">
                            @csrf
                            <button class="bg-green-500 px-3 py-1 text-white hover:bg-green-400 rounded">Accept</button>
                        </form>
                        <form action="{{ route('orders.cancel',$order->id) }}"  method="post">
                            @csrf
                            <button class="bg-red-500 px-3 py-1 text-white hover:bg-red-400 rounded">Cancel</button>
                        </form>
                    </td>
                </tr>
            @endforeach
